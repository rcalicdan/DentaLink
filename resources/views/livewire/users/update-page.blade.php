<div class="container mx-auto px-6 py-8">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-200">Update User</h1>
    ...
    <x-form.container title="User Information" subtitle="Update the details below to modify the user account"
        wire:submit="update">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="relative">
                <x-form.field label="New Password" name="password" :type="$showPassword ? 'text' : 'password'"
                    placeholder="Enter new password (leave blank to keep current)" wire:model="password"
                    icon="fas fa-lock"
                    help="Leave blank to keep current password. Must be at least 8 characters if changing." />
            ...
            <div class="relative">
                <x-form.field label="Confirm New Password" name="password_confirmation" :type="$showPassword ? 'text' : 'password'"
                    placeholder="Confirm new password" wire:model="password_confirmation" icon="fas fa-lock" />
                <button type="button" wire:click="togglePasswordVisibility"
                    class="absolute right-3 top-9 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
                    <i class="fas {{ $showPassword ? 'fa-eye-slash' : 'fa-eye' }}"></i>
                </button>
        ...
        <div class="flex justify-end space-x-3 pt-6">
            ...
            <x-utils.submit-button buttonText="Update User" wireTarget="update" />
        </div>
    </x-form.container>
</div>
